...
public class FlatLandMain extends JFrame {
		...
		for (int i = 0; i < shapes.size(); i++) {
			...
			if (shapes.size() > 1) {
				for (int j = 0; j < shapes.size(); j++) {
					Shapes ntest = shapes.get(i);
					Shapes test = shapes.get(j);
					if (ntest.hasCollided(test) && !test.equals(ntest)) {
						shapes.get(i).flipShape();
						break;
					}
				}
			}
	...
	static class Action3 implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			Triangle t = new Triangle(x, y, 50, coloring); // y=500
			shapes.add(t);
		}
	}
	...
	static class Action4 implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			Square s = new Square(x, y, 130, coloring);
			shapes.add(s);
		}
	}
